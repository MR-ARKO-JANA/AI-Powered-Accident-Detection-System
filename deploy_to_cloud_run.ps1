...
if (-not $ProjectId) {
    Write-Host "⚠️ No default GCP project found in gcloud config." -ForegroundColor Yellow
...
Write-Host "✅ Required APIs enabled successfully." -ForegroundColor Green
...
Write-Host "`n🖥️ [Step 1/3] Building & Deploying Node.js Backend..." -ForegroundColor Cyan
$BackendImageTag = "$Region-docker.pkg.dev/$ProjectId/$RepoName/backend:latest"

# Prompt user for Mongo URI configuration
Write-Host "-----------------------------------------------------------------" -ForegroundColor Yellow
Write-Host "💡 Database Setup: Google Cloud Run requires an external database." -ForegroundColor Yellow
...
if (-not $MongoUri) {
    Write-Host "⚠️ No MONGO_URI provided. Defaulting to local fallback (DB operations will fail if unreachable)." -ForegroundColor Yellow
    $MongoUri = "mongodb://localhost:27017/accident_detection"
}

# Run Cloud Build for Backend
Write-Host "🏗️ Submitting backend container to Google Cloud Build..." -ForegroundColor Yellow
...
Write-Host "🏗️ Submitting AI service container to Google Cloud Build..." -ForegroundColor Yellow
...
Write-Host "✅ AI Service deployed successfully! URL: $AiServiceUrl" -ForegroundColor Green
...
Write-Host "🏗️ Submitting frontend container to Google Cloud Build (this compiles React production bundle)..." -ForegroundColor Yellow
...
Write-Host "🖥️ Node.js Backend URL     : $BackendUrl" -ForegroundColor Cyan
